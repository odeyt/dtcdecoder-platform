...
describe("buildRankedHypotheses", () => {
  it("re-sorts by categorical confidence level, high first, re-numbering ranks", () => {
    const result = buildRankedHypotheses(
      output([
        { cause: "Crank sensor", confidenceLevel: "low", rationale: "r1", supportingEvidence: [], contradictingEvidence: [], confirmationTestsRequired: [] },
        { cause: "Open ground G103", confidenceLevel: "high", rationale: "r2", supportingEvidence: ["P0562 present"], contradictingEvidence: [], confirmationTestsRequired: ["Ohm test"] },
        { cause: "ECM internal failure", confidenceLevel: "medium", rationale: "r3", supportingEvidence: [], contradictingEvidence: [], confirmationTestsRequired: [] },
      ]),
      evidence,
    );
  ...
  it("links supportingEvidenceIds via conservative substring match on the evidence's own value", () => {
    const result = buildRankedHypotheses(
      output([
        { cause: "Low system voltage", confidenceLevel: "high", rationale: "r", supportingEvidence: ["Code P0562 was recorded"], contradictingEvidence: [], confirmationTestsRequired: [] },
      ]),
      evidence,
    );
  ...
  it("never matches evidence whose value is not actually mentioned in the AI's text", () => {
    const result = buildRankedHypotheses(
      output([
        { cause: "Unrelated cause", confidenceLevel: "low", rationale: "r", supportingEvidence: ["Nothing specific mentioned"], contradictingEvidence: [], confirmationTestsRequired: [] },
      ]),
      evidence,
    );
  ...
  it("derives evidence strength: contradicting >= supporting is weak, 2+ supporting is strong, exactly 1 is moderate", () => {
    const result = buildRankedHypotheses(
      output([
        { cause: "A", confidenceLevel: "low", rationale: "r", supportingEvidence: ["one"], contradictingEvidence: ["one", "two"], confirmationTestsRequired: [] },
        { cause: "B", confidenceLevel: "high", rationale: "r", supportingEvidence: ["P0562 present", "1HGCM82633A004352 confirmed"], contradictingEvidence: [], confirmationTestsRequired: [] },
        { cause: "C", confidenceLevel: "medium", rationale: "r", supportingEvidence: ["P0562 present"], contradictingEvidence: [], confirmationTestsRequired: [] },
      ]),
      evidence,
    );
  ...
  it("always leaves per-hypothesis missingEvidence empty (a case-level concept, not attributed per-hypothesis)", () => {
    const result = buildRankedHypotheses(
      output([{ cause: "A", confidenceLevel: "high", rationale: "r", supportingEvidence: [], contradictingEvidence: [], confirmationTestsRequired: [] }]),
      evidence,
    );
  ...
  it("carries confirmationTestsRequired through as requiredTests", () => {
    const result = buildRankedHypotheses(
      output([{ cause: "A", confidenceLevel: "high", rationale: "r", supportingEvidence: [], contradictingEvidence: [], confirmationTestsRequired: ["Fuel pressure test"] }]),
      evidence,
    );
