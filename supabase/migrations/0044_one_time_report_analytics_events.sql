-- Professional Diagnostic Report one-time-purchase funnel events
-- (docs/billing/ONE_TIME_PROFESSIONAL_REPORT.md, src/lib/analytics/events.ts)
-- added to the existing analytics_events ledger (migration 0027, last
-- extended by 0042). Purely additive — widens the event_type check
-- constraint only; no existing row's event_type value is affected.
--
-- Idempotent, same "drop if exists then unconditional add" pattern as
-- 0030/0042 — rerunning this file is always safe.
